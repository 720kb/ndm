...
.controller('ShellController', /*@ngInject*/ function HomeController($rootScope, $scope, $window, $log, $document, npm, npmGlobal, errorsService, loadingFactory) {
  const localStorageName = 'projects'
    ...
    , savedProjects = (() => {
        //auto-filter session stored projects with paths that exists (on app start up)
        let toReturnProjects = []
          , sessionProjects = JSON.parse($window.localStorage.getItem(localStorageName));
        if (sessionProjects &&
          sessionProjects.length > 0) {
          sessionProjects.forEach(item => {
            let isPath;
            ...
            if (item &&
              item.path) {
                try {
                  ...
                  isPath = false;
                  $log.warn(`Unable to read project path: ${excp}`);
                }
                if (isPath) {
                  toReturnProjects.push(item);
                }
              }
          });
        }
        $window.localStorage.setItem(localStorageName, JSON.stringify(toReturnProjects));

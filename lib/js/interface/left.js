  ...
  .controller('LeftBarController', /*@ngInject*/ function LeftBarController($rootScope, $scope, $window, $filter, $log, npm, npmGlobal, errorsService, loadingFactory, notificationFactory, appHistoryFile) {
    ...
    , unregisterLeftBarSelectedProjectNpmUnshrinkwrap = $rootScope.$on('left-bar:selected-project-npm-unshrinkwrap', (eventInformation, data) => {
        ...
        rmRf(Path.join(folder, 'npm-shrinkwrap.json'), errRm => {
          ...
          $scope.$apply(() => {
            this.shrinkwrappedProjects[projectName] = false;
          });
    ...
    , unregisterLeftBarSelectedProjectNpmShrinkwrap = $rootScope.$on('left-bar:selected-project-npm-shrinkwrap', (eventInformation, data) => {
        ...
        $scope.$apply(() => {
          ...
          this.shrinkwrappedProjects[projectName] = true;
        });
    ...
    //used to show shrinkwrap folder lock icon
    this.shrinkwrappedProjects = [];

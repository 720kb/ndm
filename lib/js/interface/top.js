...
.controller('TopMenuController', /*@ngInject*/ function TopMenuController($rootScope, $scope, $log, npm, npmGlobal, loadingFactory, notificationFactory, errorsService) {
        ...
        , installLaunch = () => {
          ...
          .then(npmInFolder =>
            ...
            this.newPackageVersion).then(() => {
              ...
              loadingFactory.finished();
              this.installingPackage = false;
              notificationFactory.notify(`Installed ${packageName}.`);
  ...
  this.uninstallPackage = currentSelectedPackages => {
    ...
    let promiseSequence = Promise.resolve()
      , npmPromise
      , uninstallingNpmGlobally
      , buttons = ['Cancel', 'Continue']
      , confirmNpmGlobalUnistall;

    //if uninstalling npm from global ALERT user
    currentSelectedPackages.forEach(item => {
      if (item.name === 'npm') {
        uninstallingNpmGlobally = true;
      }
    });
    //if uninstalling npm from global ALERT user
    if (uninstallingNpmGlobally) {
      confirmNpmGlobalUnistall = dialog.showMessageBox({
        'type': 'warning',
        'message': 'Uninstalling npm globally could cause problems on your machine.',
        'detail': 'Are you sure you want to continue?',
        buttons
      });

      if (!confirmNpmGlobalUnistall ||
        confirmNpmGlobalUnistall === 0) {
        $rootScope.$emit('top-bar:cancel-uninstall');
        this.activeLink = false;
        return $log.info('Canceled global npm uninstallation', confirmNpmGlobalUnistall);
      }
    }

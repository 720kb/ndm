...
.controller('TopMenuController', /*@ngInject*/ function TopMenuController($document, $rootScope, $scope, $log, npm, npmGlobal, loadingFactory, notificationFactory, errorsService) {

  const unregisterOnLeftBarEditProject = $rootScope.$on('left-bar:edit-project', () => {
    $scope.$evalAsync(() => {
  ...
  , unregisterLeftBarSelectProjectListener = $rootScope.$on('left-bar:select-project', (eventInformation, data) => {
    if (data &&
      data.path) {
      this.activeLink = undefined;

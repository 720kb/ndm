...
.controller('ContentController', /*@ngInject*/ function ContentController($rootScope, $scope, $log, npmGlobal, npm, loadingFactory) {
  ...
  const unregisterTopBarActiveLinkListener = $rootScope.$on('top-bar:active-link', (eventInfo, data) => {
    ...
    , unregisterInstallVersionPackageShowActiveLinkListener = $rootScope.$on('top-bar:installing-version-package', () => {
        //if selected package and clicked "install release" and clicked prompt dialog button "ok" (while literally installing package new release)
        this.showLoadingSelectedRow = true;
      })
  ...
  $scope.$on('$destroy', () => {
    ...
    unregisterInstallVersionPackageShowActiveLinkListener();

...
angular.module(moduleName, [])
  .controller('LeftBarController', /*@ngInject*/ function LeftBarController($rootScope, $scope, $window, $filter, $log, npm, npmGlobal, errorsService, loadingFactory, notificationFactory, appHistoryFile) {
    ...
    , unregisterLeftBarSelectedProjectNpmRunScript = $rootScope.$on('left-bar:selected-project-npm-run-script', (eventInformation, data) => {
      let deleteRunningScript = (dirName, scriptName) => {
        //remove running script from view
        if (this.npmRunningScriptsProject[dirName][scriptName]) {
          $scope.$apply(() => {
            delete this.npmRunningScriptsProject[dirName][this.npmRunningScriptsProject[dirName].indexOf(scriptName)];
          });
        }
      };
      if (data &&
        ...
        data.script &&
        data.scriptName &&
      !this.npmRunningScriptsProject[data.dirName][data.scriptName]
      ) {
        ...
        /*npm.inFolder(data.scriptName, data.dirName).then(() => {
          $log.info(`Finished running: npm run ${data.scriptName}`, `in folder: ${data.dirName}`);
          deleteRunningScript(data.dirName, data.scriptName);
        }).catch(error => {
          errorsService.showErrorBox('Error', `Error running script "${data.scriptName}" in folder ${data.dirName}:\n${error}`);

  ...
  return (scope, element, attrs) => {
    ...
    scope.installPackage = (pkgs, packageKind) => {
      if (pkgs &&
        ...
        if (scope.projectPath === npmGlobal) {

          npmPromise = npm.npmGlobal();
        } else {

          npmPromise = npm.npmInFolder(scope.projectPath);
        }
        ...
        if (scope.projectPath === npmGlobal) {
          ...
          fileSystem.readFile(Path.join(scope.projectPath, 'package.json'), 'utf8', (err, packageJson) => {
            if (err ||
              !packageJson ||
              (packageJson &&
              ...
              $log.info(`Writing a package.json file in folder ${scope.projectPath}`);
              //create package.json if not exist or if exist and not well formatted
              fileSystem.writeFile(Path.join(scope.projectPath, 'package.json'), '{"name": "ndm-created-project", "description": "add-a-description", "license": "add-a-license", "respository": "add-a-repository"}', error => {
                if (error) {
                  installLaunch();
                  $log.warn(`Unable to create package.json in: ${scope.projectPath}`);
                } else {
                  installLaunch();
                  $log.info(`Initialized a package.json file in ${scope.projectPath}`);
                }
              });
            } else {
              installLaunch();
    ...
    scope.search = keyword => {
      ...
      if (keyword &&
        ...
        searchTimeout = $timeout(() => {
          ...
          scope.searchResults = [];
          npm.npmInFolder(scope.projectPath).then(npmInFolder => {

...
.controller('ShellController', /*@ngInject*/ function HomeController($rootScope, $scope, $window, $log, $document, npm, npmGlobal) {
  ...
  this.enableGlobal = () => {
    ...
    const buttons = ['Cancel', 'Move global folder', 'Fix permissions'];

    dialog.showMessageBox({
      'title': 'Global Access',
      'message': 'ndm wants to access your global folder',
      'detail': 'You must choose between these two methods in order to make ndm able to manage your global folder. Click "Cancel" to dismiss.',
      buttons
    }, buttonIndex => {
      //move global folder
      if (buttonIndex === 1) {
        npm.changeGlobalFolder().then(() => {
          ...
          dialog.showErrorBox('Error', `Not able to change global folder for npm: ${error}`);
        });
      }
      //fix permissions
      if (buttonIndex === 2) {
        ...
        npm.changeGlobalFolder().then(() => {
          ...
          dialog.showErrorBox('Error', `Not able to change permissions in global npm folder: ${error}`);
        });
      }
    });
  };

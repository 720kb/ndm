      ...
      promiseSequence = promiseSequence.then(() => {
        return npmPromise.then(npmInFolder => {
          ...
          return npmInFolder.update(pkg).then(() => {
            ...
            $log.error(`Error updating ${pkg.name}...`, error);
          });
        }).catch(error => {
            $log.error(`Error preparing to update ${pkg.name}...`, error);
        });
      });
    });
      ...
      promiseSequence = promiseSequence.then(() => {
        return npmPromise.then(npmInFolder => {
          ...
          return npmInFolder.installLatest(pkg).then(() => {
            ...
            $log.error(`Error updating to latest ${pkg.name}...`, error);
          });
        }).catch(error => {
            $log.error(`Error preparing to update to latest ${pkg.name}...`, error);
        });
      });
    });
      ...
      promiseSequence = promiseSequence.then(() => {
        return npmPromise.then(npmInFolder => {
          ...
          return npmInFolder.rm(pkg).then(() => {
            ...
            $log.error(`Error uninstalling ${pkg.name}...`, error);
          });
        }).catch(error => {
            $log.error(`Error preparing to uninstall ${pkg.name}...`, error);
        });
      });
    });

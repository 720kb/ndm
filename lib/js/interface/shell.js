...
.controller('ShellController', /*@ngInject*/ function HomeController($rootScope,
  ...
  const chooseProjectDir = files => {
      ...
      if (files &&
        ...
        files.forEach(folder => {
          ...
          if (folder) {
            ...
            //automatically open the first project
            if (folder &&
               files[0] &&
               !files[1]) {
    ...
    , unregisterNpmReady = $rootScope.$on('npm:ready', () => {
      //is npm installed globally?
        npm.isNpmGloballyInstalled().then(() => {
  ...
  this.updateNpm = () => {
    ...
    if (!this.updatingNpm) {
      dialog.showMessageBox({
        ...
        'message': 'Update npm',
        'defaultId': 0,
        'detail': 'Are you sure you want to update npm globally?',
        'buttons': ['Cancel', 'Yes']
      }, buttonIndex => {
        if (buttonIndex === 1) {

          this.activeClickedLink('update');
  ...
  this.updateNpmBadgeVersion = () => {
    if (!this.updatingVersionBadge) {
      ...
      npm.outdatedGlobalVersion().then(data => {
        if (data &&
          data.length > 0) {

          const globalNpm = data
            .filter(element => element.name === 'npm')
            .reduce((prev, current) => {
          ...
          $scope.$apply(() => {
            ...
            this.npmLatestVersionBadge = globalNpm.latest || undefined;
            this.npmCurrentVersionBadge = globalNpm.current || undefined;

...
.controller('TopMenuController', /*@ngInject*/ function TopMenuController($rootScope, $scope, $log, npm, npmGlobal, loadingFactory, errorsService) {
    ...
    let npmPromise
      ...
      , installLaunch = () => {
        ...
        npmPromise.catch(error => {
          ...
          errorsService.handleError('Error', `Configuring npm for installing ${packageName}...`, error);
        })
        .then(npmInFolder =>
          npmInFolder.install({
            'name': packageName,
          ...
          this.newPackageVersion).then(() => {
            ...
            errorsService.handleError('Error', `Installing ${packageName}: ${error}`);
          }));
      };
  ...
  this.installVersionPackage = (currentSelectedPackage, specificVersion) => {
    ...
    npmPromise.catch(error => {
      ...
      errorsService.handleError('Error', `Error configuring npm for installing ${currentSelectedPackage.name}: ${error}`);
    }).then(npmInFolder => {
      ...
      .then(() => {
        ...
        errorsService.handleError('Error', `Error installing ${currentSelectedPackage.name}: ${error}`);
      });
    });
  };

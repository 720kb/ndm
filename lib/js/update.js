          ...
          this.url = updateMetaInfo.assets
            .filter(element => {
              if (process.platform === 'darwin') {
                ...
                return element.name.includes('mac.zip');
              } else if (process.platform === 'win32') {

                return element.name.includes('win.zip');
              } else if (process.platform === 'linux' &&
                !element.name.includes('mac.zip') &&
                !element.name.includes('win.zip')) {

                return element.name.includes('.zip');
              }
            })
    ...
    , onUpdateEnd = event => {
      ...
      fs.mkdtemp(tmpFolder, (err, folder) => {
        ...
        if (process.platform === 'darwin') {
          ...
          newResources = path.resolve(folder, path.join('linux-unpacked', 'resources', 'app'));
        }
        ...
        fs.appendFile(fileToSave, new Buffer(event.target.response), appendErr => {
          ...
          if (process.platform === 'linux') {
            ...
            sudo.exec(`mv ${newResources}/* ${remote.app.getAppPath()}`, options, () => {

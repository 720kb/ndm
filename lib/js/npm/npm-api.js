...
.service('npm', /*@ngInject*/ function NpmService($rootScope) {
  ...
  , getNpmLatestAvailableVersion = () => new Promise((resolve, reject) => {
    exec(`npm show npm version`, (err, stdout, stderr) => {
      ...
      if (err || stderr) {
        ...
        console.info('Latest npm version available is:', stdout);
      }
      return resolve(stdout);
    });
  })
  , getNpmInstalledVersion = () => new Promise((resolve, reject) => {
    exec(`npm -v`, (err, stdout, stderr) => {
      ...
      if (err || stderr) {
        ...
        console.info('Installed npm version is:', stdout);
      }
      return resolve(stdout);
    });
  })
  ...
  this.getNpmInstalledVersion = getNpmInstalledVersion;
  this.getNpmLatestAvailableVersion = getNpmLatestAvailableVersion;

...
angular.module(moduleName, [])
  .controller('LeftBarController', /*@ngInject*/ function LeftBarController($rootScope, $scope, $window, $filter, $log, npm, npmGlobal, errorsService, loadingFactory, notificationFactory, appHistoryFile) {
    ...
    , unregisterLeftBarSelectedProjectNpmReinstall = $rootScope.$on('left-bar:selected-project-npm-reinstall', (eventInformation, data) => {
        ...
        rmRf(`${folder}/node_modules`, errRm => {
          if (errRm) {
            ...
            return errorsService.showErrorBox('Warning', `Error deleting node_modules/ in folder ${folder} for clean installation: \n${errRm}`);
          }
    ...
    , unregisterLeftBarSelectedProjectNpmRunScript = $rootScope.$on('left-bar:selected-project-npm-run-script', (eventInformation, data) => {
      let deleteRunningScript = indexName => {
        //remove running script from view
        if (this.npmRunningScriptsProject[data.dirName][indexName]) {
          $scope.$apply(() => {
            this.npmRunningScriptsProject[data.dirName].splice(this.npmRunningScriptsProject[data.dirName].indexOf(indexName), 1);
          });
        }
      };
      ...
      if (data &&
        ...
        exec(`npm run ${data.scriptName}`, {'cwd': data.dirName}, (error, stdout, stderr) => {
          if (error) {
            deleteRunningScript(data.scriptName);
            errorsService.showErrorBox('Error', `Error running script "${data.scriptName}" in folder ${data.dirName}:\n${error}`);
            return $log.error(`Error running script "${data.scriptName}" in folder ${data.dirName}:\n${error}`);
          }

          deleteRunningScript(data.scriptName);
          $log.info(`Finished running: npm run ${data.scriptName}`, `In folder: ${data.dirName}`, stdout, stderr);
        });
      }
    });

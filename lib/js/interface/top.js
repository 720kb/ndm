...
.controller('TopMenuController', /*@ngInject*/ function TopMenuController($rootScope, $scope, $log, npm, npmGlobal, loadingFactory) {
  ...
  this.updateNpmBadgeVersion = () => {
    ...
    npm.getNpmInstalledVersion().then(currentVersion => {
      npm.getNpmLatestAvailableVersion().then(availableVersion => {

        if (currentVersion === availableVersion) {

          $scope.$apply(() => {
            this.npmVersionBadge = undefined;
          });
        } else {
          $scope.$apply(() => {
            this.npmVersionBadge = availableVersion;
          });
        }
      });
    });

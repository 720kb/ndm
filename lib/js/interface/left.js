  ...
  .controller('LeftBarController', /*@ngInject*/ function LeftBarController($rootScope, $scope, $window, $filter, $log, npm, npmGlobal, errorsService, loadingFactory, notificationFactory, appHistoryFile) {
    ...
    this.deleteSnapshot = () => {
      ...
      Storage.get(appHistoryFile, (err, data) => {
        if (err) {
          ...
          data[projectKey].filter(item => {
            return item.datetime !== selectedSnapshot.datetime;
          }).map(mapElement => ({
              'datetime': mapElement.datetime,
              'status': mapElement.status
          }));
          ...
          if (err) {
            ...
            if (storageData &&
              ...
              storageData[storageKey].push({
                'datetime': $filter('date')(new Date(), 'medium'),
    ...
    this.openHistoryPrompt = project => {
      Storage.get(appHistoryFile, (err, data) => {
        ...
        this.projectHistory = data[project.path.toString().toLowerCase().trim().replace(/\//g, '-')];
        $scope.$apply(() => {

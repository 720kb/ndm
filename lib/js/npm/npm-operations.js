...
class NpmOperations {
  ...
  search(keyword) {
    ...
    //this[npmSym].config.set('searchopts', '--json --parseable');
    this[npmSym].config.set('json', true);
    this[npmSym].config.set('parseable', true);
    return new Promise((resolveSearch, rejectSearch) => {
      this[npmSym].commands.search([keyword], err => {

        if (err) {
          this[npmSym].config.set('json', false);
          this[npmSym].config.set('parseable', false);
          return rejectSearch(err);
        }
